...
func testUserToOneApplicationUsingApp(t *testing.T) {
	...
	if err = local.L.LoadApp(tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.App == nil {
...
func testUserToOneSetOpApplicationUsingApp(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `int`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `DeletedAt`: `timestamp`, `Name`: `varchar`, `Password`: `varchar`, `Verified`: `tinyint`, `Avatar`: `varchar`, `UserID`: `varchar`, `UserIDUpdated`: `tinyint`, `AppID`: `varchar`}
	_           = bytes.MinRead
)
